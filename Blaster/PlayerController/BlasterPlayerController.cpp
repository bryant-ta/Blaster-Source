...
// Manage high ping warning animation
...
// Control CharacterOverlay (HUD) health display
...
// Control CharacterOverlay (HUD) score display
...
void ABlasterPlayerController::SetHUDMatchCountdown(float CountdownTime)
{
	...
	{
		if (CooldownTime < 0.f) // time is negative briefly between loading match states, hide text in that case
		{
...
{
	if (CharacterOverlay == nullptr) // poll init CharacterOverlay bc BlasterHUD is not ready yet since it lives in Controller
	{
...
// transition to inprogress (in match) state
void ABlasterPlayerController::HandleMatchHasStarted()
{
	BlasterHUD = BlasterHUD == nullptr ? Cast<ABlasterHUD>(GetHUD()) : BlasterHUD;
	if (BlasterHUD)
	{
		// add character hud overlay and hide announcement
...
void ABlasterPlayerController::HandleCooldown()
{
	...
	if (BlasterHUD)
	{
		...
		{
			// set cooldown state texts, such as top scoring player
